...
const Contacts = () => {
  ...
  navigate(`/actIndumentaria/${id_indu}/${concepto_indu}/${ubicacion_indu}/${area_indu}/${id_laboratorio_indu}/${estado_indu}`); 
  }

  useEffect(() => {
    // Utiliza el hook useEffect para realizar la solicitud a la API al cargar el componente
    axios
      .get("https://cloud-service-leonardo13344.cloud.okteto.net/indumentaria") // Reemplaza "http://ruta-de-tu-api.com/pacientes" con la URL de tu API
      .then((response) => {
  ...
  const handleEstadoButtonClick = (row) => {
    ...
    axios
      .put(`https://cloud-service-leonardo13344.cloud.okteto.net/indumentaria/${id}`, updatedData)
      .then((response) => {
