...
const Contacts = () => {
  ...
  navigate(`/actEspecialidad/${id_espe}/${encargado_espe}/${descripcion_espe}/${id_departamento_espe}/${estado_espe}`); 
  }
  useEffect(() => {
    // Utiliza el hook useEffect para realizar la solicitud a la API al cargar el componente
    axios
      .get("https://cloud-service-leonardo13344.cloud.okteto.net/especialidades") // Reemplaza "http://ruta-de-tu-api.com/pacientes" con la URL de tu API
      .then((response) => {
  ...
  const handleEstadoButtonClick = (row) => {
    ...
    axios
      .put(`https://cloud-service-leonardo13344.cloud.okteto.net/especialidades/${id}`, updatedData)
      .then((response) => {
