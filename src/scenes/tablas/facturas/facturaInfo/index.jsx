...
const Contacts = () => {
  ...
  navigate(`/actFactura/${id_fact}/${fecha_emision_fact}/${paciente_fact}/${descripcion_fact}/${monto_fact}
  /${metodo_pago_fact}/${id_receta_fact}/${estado_fact}`); 
  }
  useEffect(() => {
    // Utiliza el hook useEffect para realizar la solicitud a la API al cargar el componente
    axios
      .get("https://cloud-service-leonardo13344.cloud.okteto.net/facturas") // Reemplaza "http://ruta-de-tu-api.com/pacientes" con la URL de tu API
      .then((response) => {
  ...
  const handleEstadoButtonClick = (row) => {
    ...
    axios
      .put(`https://cloud-service-leonardo13344.cloud.okteto.net/facturas/${id}`, updatedData)
      .then((response) => {
